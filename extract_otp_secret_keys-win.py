    ...
    for otp in payload.otp_parameters:
        ...
        if args.htmlfile:
            with open(args.htmlfile, 'a') as f2:
                ...
                system("SimpleCodeGenerator.exe /Save " +
                       keyURI + " img/"+str(line_count+j)+".png")
                encoded = base64.b64encode(
                    open("img/"+str(line_count+j)+".png", "rb").read())
                encoded2 = encoded.decode('utf-8')
                remove("img/"+str(line_count+j)+".png")
                f2.write('TOTP Profile '+str(line_count+j)+':'+otp.name +
                         "<br><img width=250  src='data:image/png;base64,"+encoded2+"'><pre>"+secret+"</pre><hr><br><br>")
# empty input_qr.txt
